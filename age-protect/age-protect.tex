...
\begin{itemize}
	...
	\item \textbf{Respects the privacy} and anonymity of all participants. Age Protect is a local solution. The user places credentials into their wallet as one interaction, and then as some future point initiates a completely separate interaction to present these credentials to an app or site. These two interactions cannot be linked by the issuer of the credentials or the recipient app/site. There is no back channel between the issuer and the recipient apps/sites.
