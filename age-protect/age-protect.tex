...
In addition to traditional digital wallet functions, an Age Protect-compatible wallet is discoverable by sites and apps. The signaling from the wallet and the detection of this signal from the app/site rely on the Agent Presentation \colorbox{BurntOrange}{[missing reference]} protocol. Using Agent Presentation, for web usage the wallet would integrate with the browser and include the \emph{Agent} HTTP field.
