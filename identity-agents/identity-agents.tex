...
Data sharing with a local identity agent is straightforward; with the user's consent the local agent simply shares data in its personal datastore directly with the provider app or website. Data sharing with a virtual agent is a bit more complicated since the central virtual agent doesn't directly manage or persist contextualized user data. In the virtual case, a provider's shard requests data (e.g. to fill in a form on the provider app/site) from another shard within another provider. If the user authorizes this transfer from the source provider shard to the destination shard, a \hyperfootnote[capability][https://]{en.wikipedia.org/wiki/Capability-based\_security} is generated by the provider shard and given to the destination shard, so it can be used to request the relevant attribute from the source shard. 
