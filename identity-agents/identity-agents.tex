...
In the data flows just mentioned, the user is relatively powerless over their data. They have little or no visibility into what's being gathered, where it's being shared and how it's used. Users live in what Alicia Solow-Niederman calls an ``inference economy''\cite{Solow-Niederman2022} wherein big data and machine learning are used to infer traits that form new kinds of personal information--often more sensitive than the underlying source data. Harm and risk can rarely be evaluated outside a specific situation\cite{Solove2023}, yet it is useful to list a few representative types of harm. Individuals, are vulnerable to data breaches, they can be spammed by marketers, they are vulnerable to identity theft, they can be exposed to price and/or hiring discrimination, and they can be stalked. 

``Debates over privacy are really debates about how power will be allocated in an information society and how much power the humans in that society will get as consumers or citizens.''\cite{Richards2021} Today, despite significant new regulation, the basic approach to protect privacy hasn't changed since the 1970s. It is often called \emph{notice and consent}. Solove described it using the term \emph{privacy-self management}, as follows:
...
Unfortunately, an individual's account identifiers and associated human data are bound to specific online service providers and can't be moved freely from one to another. In other words they are not \emph{portable}.\footnote{Efforts such as the Data Transfer Institute (https://dtinit.org) are relevant here.}
...
For the past two decades, hundreds of independent developers, and organized groups have explored different paths to restore the power imbalance we've described. We mention a few of them here.

\emph{Personal agents:\footnote{What Mozilla calls \emph{user agents} (https://developer.mozilla.org/en-US/docs/Glossary/User\_agent)}}: Software tools that work (i.e. provide agency and power) ``on the individual's side"\footnote{Project VRM\cite{Searls2019} refers to this as ``tools for individuals to manage relationships with organizations'' to which we would add ``...or with other individuals."} for, and \emph{exclusively} on behalf of, the person. Projects include: \hyperfootnote[ProjectVRM][https://]{projectvrm.org/}, \hyperfootnote[DataPal][https://]{datapal.me}, and \hyperfootnote[Customer Futures][https://]{customerfutures.com}.

\emph{\hyperfootnote[Personal datastores][https://]{https://wikipedia.org/wiki/Personal\_data\_service} and human centric  personal data management:} \hyperfootnote[Solid Project][https://]{https://solidproject.org}, \hyperfootnote[Mydex][https://]{mydex.org}, and the \hyperfootnote[My Data Foundation][https://]{mydata.org}. 

\emph{Re-decentralizing the internet:} \hyperfootnote[redecentralize.org][https://]{redecentralize.org}, \hyperfootnote[nlnet.nl][https://]{nlnet.nl}, \hyperfootnote[DWeb principles][https://]{getdweb.net/principles/}, \hyperfootnote[The Web3 Foundation][https://]{web3.foundation/}, Berners-Lee's \hyperfootnote[Decentralized Information Group][https://]{dig.csail.mit.edu}. 

\emph{Data portability and interoperability between service providers using both policy and technical means:} \cite{Doctorow2021}, \cite{Cyphers2021}, \hyperfootnote[Data Transfer Initiative][https://]{dtinit.org/}.

\emph{Decentralized and self-sovereign identity:} \hyperfootnote[the Decentralized Identity Foundation(DIF)][https://]{identity.foundation} and \emph{self-sovereign identity}\cite{Preukschat2021} movement. For an overview, see \hyperfootnote[The 10 principles of Self-Sovereign Identity (SSI)][https://]{www.selfsovereignidentity.it/the-10-principles-of-self-sovereign-identity-ssi/}. 

\emph{Local-first software\hyperfootnote[principles][https://]{inkandswitch.com/local-first/}:}\cite{Kleppmann2019}.
...
People are messy and life is complicated. We choose to express different parts of ourselves differently in different contexts and society assumes we will do so. 
...
We share different attributes about ourselves in different contexts. Sometimes the value of single specific attribute may differ by context. Let's look at how a person's age varies by context. They might share chronological age among their close friends, a fictional age to a music recommendation service, no age at all in contexts wherein doing so might expose them to discrimination, and share merely a claim that they exceed the minimum legal drinking age. 

In his \hyperfootnote[last public speech][https://]{www.youtube.com/watch?v=9DExNTY3QAk}  
\hyperfootnote[Kim Cameron][https://]{en.wikipedia.org/wiki/Kim\_Cameron\_(computer\_scientist)} introduced two useful definitions based on archaic English terms:
...
These relays have their own privacy, autonomy, trust, and security considerations. We mention a few of them here.  Technically competent individuals can host their own relay servers, presuming it is relatively straightforward to build and deploy. Everyone else will have to rely on an external administrative hosting authority. Ideally they will be available freely or at low cost. In either the self-hosted or external case, the relay needs to be trusted. For this reason its source code should be open. The relay should only handle encrypted data transiently. That is, it should temporarily buffer (encrypted) message data while waiting for the recipient app or agent to come back online.
...
Apps can request personal information from, and provide information to, the user's identity agent using PDN protocols (see section \ref{sec:PDN}). This information may flow between the app and the identity agent in one direction, the other, or in both. The app reads and writes data using the identity agent's data model. This data may include both structured and unstructured data and may include digitally signed documents (e.g. Verifiable Credentials, etc.). 
\begin{comment}
...
\textbf{Signaling intent}

An identity agent can signal a person's intent to apps. Examples of intent include (i) exercising the person's legal privacy right opt out of apps sell and/or sharing their personal information using the Global Privacy Control\footnote{\url{https://globalprivacycontrol.org}}, and (ii) allowing the person to proffer their privacy requirements as contractual terms using IEEE 7012.\footnote{\url{https://standards.ieee.org/ieee/7012/7192/}}. \hyperfootnote[MySignals][https://]{mysignals.org} describes an extensible framework agents can use for signaling.

\textbf{Password-less login}

Identity agents enable a person to log in to apps using a variety of password-less authentication technologies. The identity agent knows who the user is because the user authenticates to it, so the identity agent can represent the user in their interactions with apps, and can do so without revealing correlatable identifiers. This is both private and convenient.

\textbf{Infer ad profiles}

An identity agent can generate on-device an ad profile by inferring traits from the person's browsing behavior. The person can review and edit this profile, and may choose to share it with apps that are supported by interest-based \emph{private} advertising technology. This approach eliminates the need for surveillance by third-parties using cookies and other tracking technologies. It is similar in design to \hyperfootnote[Google's Topics API][https://]{developer.chrome.com/en/docs/privacy-sandbox/topics/overview/}.
...
In our example Alice has two digital relationship contexts called \emph{connections}. The blue circle at the left inside black ``Alice'' container represents Alice's selfness--her innate sense of being a single individual person. In the context of each relationship Alice may choose to express herself differently. The blue circle within the Robert Fox box represents who she is to context of the relying party named Robert Fox (i.e. her whoness to Robert). The blue circle within the Untied Airlines box represents who she is to this airline relying party. The personal information (attributes, messages, credentials, etc.) about Alice that she chooses to present may differ by context, and usually does.

\subsection{Functionality}

Figure~\ref{fig:functionality} summarizes the functionality of an identity agent. The cells in light and dark green show the progress of implementation work being led by The Mee Foundation. 
...
\begin{itemize}
\item \textbf{Organize} relationships with relying party's apps into a set of connections.
...
The terms \emph{self}, \emph{context}, and \emph{connection}, are defined in section \ref{data_model_subsection} as part of the Persona data model description.
...
This layer manages local data contexts (some of which may be accessed by Connectors), representations of the Self and the set of relying parties with which the user is connected. Data is encrypted at rest using \hyperfootnote[FIPS-compatible][https://]{www.nist.gov/standardsgov/compliance-faqs-federal-information-processing-standards-fips} algorithms. 
...
The Persona data model is used to represent personal information about the agent user and the parties with whom they have connections. Physically, data managed by one identity agent may be replicated across multiple agents on different devices, but we focus here on the logical model, not these replicas. 

These Person instances are connected into a directed graph that spans \emph{Context} containers. The singleton Self container holds a single Person node that represents the selfness of person as a single individual. The Self node links to Person nodes in distinct context containers. Each Person contains information about how the user (Self) perceived by the relying party, that is, their \emph{whoness}. 

The namespace of the identifier of the Person node in each context varies based on the digital protocol used to interact with the relying party. Since a hypothetical user named Alice's relationship with Robert is via an agent-to-agent protocol, the namespace would utilize a pseudonymous \hyperfootnote[DID URL][https://]{www.w3.org/TR/did-1.0/} identifier, exchanged during an introduction ceremony with Robert. In Alice's relationship with Untied Airlines, her Person identifier is an email address. The graph of Person nodes with identifiers is shown in Figure~\ref{fig:contexts-and-ids}
...
In each context the Person node has a set of information (attributes, messages, credentials, etc.) defined by the user. We call claims \emph{about} the user (Alice) and are made \emph{by} the user (Alice) \emph{UBU} (User-By-User) claims. For example, Alice might create the UBU claim that her given name is "Alice" and another UBU claim the value of which is her street address. In her relationship with Untied Airlines she might include her given name UBU claim of "Alice", but not choose to include a UBU claim of her home address. 
...
In the context of Alice's a relationship with a relying party (RP), the RP shares claims it makes about itself/themselves. For example in the context of Alice's connection to Robert Fox, Robert's identity agent shares claims about Robert with Alice's agent. These claims about RPs made by RPs are called \emph{RBR} claims. An example of an RBR claim is the claim made by Robert Fox that his given name is Robert. We complete our tour of claim types with fourth and final type, called \emph{RBU} claims. These are claims made about the RP by the user. RBU claims are never shared with the RP. An example of an RBU claim by Alice might be a private note to herself that she met Robert in 2019. See Figure~\ref{fig:RBRs-and-RBUs}.
...
Now that we've discussed contexts, connections and the four types of claims, we turn to aggregates of connections called \emph{Collections} and \emph{Groups}. 

A \emph{collection} is a set of 1:1 relationship connections. Collections are user-defined entities that allows the user to group, organize and move connections between collections as they please. Collections are private to the identity agent. 

Figure~\ref{fig:Collections} shows Alice as having organized her connections into two collections, \emph{Friends} and \emph{Work}. The former contains her connections to Bob Fox and Kristin Watson. The latter contains two connections to co-workers, Charlie Smith and Devon Lane as well as two connections to service providers, Google and Untied Airlines. 

\begin{figure}[h!]
\includegraphics[width=\textwidth]{./images/collections.png}
\caption{Two Collections}
\label{fig:Collections}
\end{figure}

A \emph{group}, like a collection, is a set of connections. However, whereas a collection is private, a group (i.e. the group entity itself along with all of its attributes) is replicated to the RPs of all of its connections. 

For example, if Alice has a group with connections to RPs B and C, then the group membership set, (A, B, C), is shared and synchronized to RPs B and C. Thus RP B knows that it is in a group with A and C, and RP C knows it is in a group with A and B. The group also may have a set of \emph{group attributes} which are dynamically and continuously shared with all members of the Group. A group attribute could be a shared calendar, group chat, etc. 
...
The Mee License imposes obligations on the app provider, among which is the requirement to respect the user's \emph{data rights} to access, correction (editing), and deletion of the information collected and held by them. The Mee License covers information that the user may have shared manually (e.g. by filling in a form, or other kinds of in-app interactions) or shared with them by a person's identity agent. The license requires the provider to implement \emph{data rights} APIs that an identity agent uses to remotely control this app-held data. In this way, we tie the legal (license) and technical means (identity agents and APIs) together.
