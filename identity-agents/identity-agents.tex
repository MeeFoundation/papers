...
A Protocol is an attribute of a Context. Typically, a Context has only one Protocol, although more than one is sometimes present. Figure~\ref{fig:protocol} shows an example of Alice who has a connection with LinkedIn. This connection has a single Context that contains the information that Alice shares with the bank via the OpenID Connect SIOPv2 protocol.
