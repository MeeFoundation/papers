...
The Controller subcomponent handles requests from the UI. The Controller manages the Self and the RPs data containers. It updates the attributes of Person objects in contexts via the Data Access component. It is responsible for creating and deleting connections. Lastly, it is responsible for replicating the Self and RPs data containers to other identity agent replica nodes the user may have.
...
This layer manages the one-to-many mapping of a single subject's (e.g. a user's or RP's) identity (e.g. a \hyperfootnote[DID][https://]{www.w3.org/TR/did-core/} identifier) to one or more identifiers (nodeIds) for each of the identity agent instances (nodes) owned and controlled by the subject. 
