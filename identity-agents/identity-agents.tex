...
The Controller handles requests from the UI. It manages the Self and the RPs data containers. If we consider that a single \emph{logical} identity agent of a user is represented by multiple identity agent \emph{nodes} on different devices, the Controller is responsible for replicating and synchronizing the state of the identity agent between these nodes. Note that since the storage capacity of each node may vary based on the storage capabilities of its host device, whereas all Self and RPs data is always replicated on each node, some Contexts may be omitted on the nodes running on storage-constrained devices.
...
In contrast to a local identity agent that runs entirely on a local device, a virtual agent is a distributed system consisting of a web app for the user that communicates with agent ``shard'' embedded within providers' apps. Each separate agent shard within a specific provider's app maintains the data that represents the user's relationship with that provider. 
